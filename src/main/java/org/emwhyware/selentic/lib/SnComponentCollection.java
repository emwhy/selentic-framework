 ...
 * @see ScAbstractPage
 */
public class SnComponentCollection<T extends ScComponent> implements Iterable<T> {
    ...
    private List<WebElement> webElements() {
        if (this.$callerComponent instanceof ScAbstractPage || this.selector.isAbsolute()) {
            ...
            return ((ScComponent) this.$callerComponent).existingElement().findElements(this.selector instanceof SnXPath ? ((SnXPath) this.selector).build(true) : this.selector.build());
        }
    }
