...
import org.emw.selentic.lib.exception.*;
import org.emw.selentic.lib.selector.*;
import org.emw.selentic.lib.exception.*;
import org.emw.selentic.lib.selector.*;
 ...
 * {@link ScPage} and {@link ScComponent}.
 * 
 *
...
public abstract class ScComponent extends ScAbstractComponent {
    ...
    protected final @NonNull WebElement existingElement() {
        try {
            ...
            ScWait.waitUntil(this.waitTimeout(), this::exists);
            element = webElement();
            this.verifyRules(element);
            return element;
        } catch (ScWaitTimeoutException ex) {
            throw new ScElementNotFoundException("Element does not exist.", ex);
        }
    }
     ...
     * If click needs to be public, extend from {@link ScClickableComponent} instead.
     * 
     *
     ...
     * <p>
     * This is a protected method, so it can be used within the extended component class as needed.
     * If double-click needs to be public, extend from {@link ScClickableComponent} instead.
     * 
     *
     * @throws ScElementNotFoundException if the element is not displayed
     */
    ...
    private static class SeOwnText {
        ...
        private static String removeHtml(@NonNull String originalHtmlText) {
            ...
            if (ANY_TAG.matcher(resultText).find()) {
                throw new ScInvalidHtmlException("Cannot parse HTML text: \n\n" + originalHtmlText);
            }
