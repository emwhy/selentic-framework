...
import org.selion_framework.lib.exception.SeLoggerException;
...
final class SeInitializer {
    private static final Logger LOG = SeLogHandler.logger(SeInitializer.class);

    static void init() {
        final File rootProjectDir = SeLogHandler.userDirectory();
        ...
        if (logsDir.exists()) {
            ...
            throw new SeLoggerException("Failed to create log directory " + logsDir.getAbsolutePath());
        }
        if (!currentLogDir.mkdirs()) {
            throw new SeLoggerException("Failed to create log directory " + currentLogDir.getAbsolutePath());
        }
        SeLogHandler.setLogDirectory(currentLogDir);
        LOG.info("Created log directory: {}", currentLogDir.getAbsolutePath());
        SeLogHandler.configureLogger();
