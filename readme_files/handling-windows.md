...
Using lambda expression, it neatly isolates any action that happens in another browser window.

Switching and closing extra browser windows happens automatically before and after the lambda expression. If the browser window is not closed at the end of the lambda expression, it closes it before switching to the previous window. Otherwise, it simply switch the control to the previous window. 

The code below shows **inWindow()** usage. The method can be nested for multiple browser windows.

Optionally, **controller** can be a parameter. the controller allows switching back to the previous opened window using **inOtherWindow()** method of the controller.
